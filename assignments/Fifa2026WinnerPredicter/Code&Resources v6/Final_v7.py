...
# *** UPDATED: Pointing to the new v4 (weighted) models ***
RF_MODEL_PATH = '/mnt/khome/requiem/Documents/programs/AI&ML/assignments/Fifa2026WinnerPredicter/Code&Resources v6/backenddata/rf_classifier_v4.joblib'
PREPROCESSOR_PATH = '/mnt/khome/requiem/Documents/programs/AI&ML/assignments/Fifa2026WinnerPredicter/Code&Resources v6/backenddata/preprocessor_v4.joblib'
LABEL_ENCODER_PATH = '/mnt/khome/requiem/Documents/programs/AI&ML/assignments/Fifa2026WinnerPredicter/Code&Resources v6/backenddata/label_encoder_v4.joblib'
...
class KMeansWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Parse the cluster profile summary
            for line in stdout.splitlines():
                ...
                if in_summary:
                    # Stop parsing if we hit an empty line or the team list after finding data
                    if (not line.strip() or line.strip().startswith('[')) and found_data:
                        in_summary = False
                        break 
                        
                    # This captures the data lines
            ...
            # Find the best cluster based on the first metric (xg_plus_minus_per90)
            best_cluster_id = -1
            max_metric = -float('inf')

            for line in data_lines:
                if line.strip().startswith('['): # Skip any stray list lines
                    continue 
                ...
                metric_val = float(parts[1]) # xg_plus_minus_per90
            ...
            # Now parse the team assignments
            cluster_teams = {}
            ...
            for line in stdout.splitlines():
                ...
                if line.startswith("=== Cluster"):
                    ...
                    current_cluster_id = -1 # Reset after capturing teams for the cluster
...
class BracketView(QGraphicsView):
    def __init__(self):
        ...
        self.set_theme(DARK_THEME) # Default theme
    def set_theme(self, theme):
        ...
        # Redraw if items exist
        if self.scene.items():
            pass # Redraw logic would be complex, simpler to just re-run simulation
    ...
    def draw_match(self, x, y1, y2, team1, team2, winner=None):
        ...
        if winner:
            ...
            # Make winner bold in original match
            if winner == team1:
    ...
    def fit_view(self):
        try:
            ...
            # Pass if scene is empty or has issues
            pass 
...
class OddsBarWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        color_draw = QColor("#888888") # Neutral gray for draw
        
        width_a = int(total_width * self.prob_a)
        width_draw = int(total_width * self.prob_draw)
        width_b = total_width - width_a - width_draw # Remainder
...
class DataVisualsTab(QWidget):
    ...
    def load_data(self, stats_df, column_map):
        ...
        # Get display names for feature columns that exist
        ...
        # Set default scatter plot to possession vs xg
        try:
            # Set xg as default y-axis
            xg_index = display_features.index(column_map['xg'])
            self.scatter_combo_y.setCurrentIndex(xg_index)
        except (ValueError, KeyError):
            self.scatter_combo_y.setCurrentIndex(1) # Fallback
        ...
        for i, row in self.stats_df.iterrows():
            team_name = row['team'] # Assuming 'team' is the column name
    ...
    def set_theme(self, theme):
        ...
        # Theme for the inner QTabWidget
    ...
    def init_comparison_tab(self):
        ...
        layout.addLayout(controls_layout, 1) # 1/3 of space
        
        # Right side plot
        self.comp_canvas = MplCanvas(self)
        layout.addWidget(self.comp_canvas, 2) # 2/3 of space
        
        self.visuals_tabs.addTab(tab, "Team Comparison")
        
    def _apply_plot_theme(self, fig, ax):
        """Helper function to apply current theme to a Matplotlib fig/ax."""
    ...
    def plot_heatmap(self):
        ...
        try:
            ...
            # Use only feature columns for correlation
    ...
    def plot_team_comparison(self):
        ...
        try:
            ...
            if not selected_teams:
                # Clear plot if no teams are selected
...
class FootballPredictorApp(QWidget):
    ...
    def init_ui(self):
        ...
        self.visuals_tab = DataVisualsTab() # Use the new class

        # Add Tabs
        self.tabs.addTab(self.simulator_tab, "Tournament Simulator")
        self.tabs.addTab(self.stats_tab, "Team Data") # Old stats table
        self.tabs.addTab(self.odds_tab, "Match Odds")
        self.tabs.addTab(self.visuals_tab, "Data Visuals") # New advanced visuals
    ...
    def init_simulator_tab(self):
        ...
        # Left Panel (Controls & Team Selection)
        ...
        simulator_tab_layout.addLayout(left_panel, 2) # Left panel takes 2 parts of width
        
        # Right Panel (Log & Bracket)
        ...
        simulator_tab_layout.addLayout(right_main_panel, 5) # Right panel takes 5 parts
    ...
    def apply_stylesheet(self, theme):
        ...
        # Re-theme existing odds widgets
    ...
    def load_stats_data(self):
        try:
            ...
            # --- Load data into Data Visuals Tab ---
            ...
            if self.stats_table_view:
                self.stats_table_view.setParent(None) # Remove broken table

    def load_models_and_data(self):
        try:
            # Note: These paths now point to v4 from the constants
            ...
            self.output_info.append(f"Weighted prediction models (v4) and lookup data loaded successfully.")
            
            # Start the K-Means worker thread
    ...
    def on_kmeans_finished(self, top_teams):
        ...
        self.team_grid_label.setText(f"Team Pool ({len(self.top_teams_pool)} Elite Teams):")
        self.selected_teams_label.setText(f"Selected Teams (Pick 8):")
        ...
        self.output_info.append(f"\nYour {len(self.top_teams_pool)} elite teams are loaded.\n\n"
                                ...
                                "2. Or, click 'Randomize Bracket' to pick 8 at random.")
    ...
    def update_team_buttons(self):
        ...
        # Create new buttons for the loaded teams
    ...
    def team_button_clicked(self, team_name):
        if team_name in self.selected_tournament_teams:
            # Team is already selected -> De-select it
            ...
            # Team is not selected -> Select it
            if len(self.selected_tournament_teams) < 8: # Only allow 8 teams
                ...
                self.output_info.append(f"Cannot add team: You have already selected 8 teams.")
    ...
    def auto_select_teams(self):
        ...
        # Sample 8 teams from the pool
        selected_teams = random.sample(self.top_teams_pool, 8)
        
        # Use a timer to add teams sequentially for a nice visual effect
        ...
        self.auto_select_timer.start(150) # Add one team every 150ms
    ...
    def predict_match(self, team_a, team_b, is_knockout):
        ...
        try:
            # 1. Get win rates
            team_a_rate = self.team_win_rate_lookup.get(team_a, 0.5) # Default to 0.5 if not found
            ...
            is_home_advantage = 0 # Assume neutral venue
    ...
    def get_winner(self, prediction_result):
        # Determine winner from probabilities
    ...
    def start_tournament_simulation(self):
        if len(self.selected_tournament_teams) != 8:
            self.output_info.append("Tournament start failed: 8 teams must be selected.")
        ...
        for i, (team_a, team_b) in enumerate(qf_matches):
            ...
            # Draw this match on the bracket
            y1 = qf_y_positions[i]
            y2 = y1 + 40 
        ...
        # Fit view to show the whole bracket
        self.bracket_view.fit_view()
        
        # Populate the odds tab
    ...
    def clear_odds_widgets(self):
        # Clear all widgets from the odds layout
    ...
    def reset_all(self):
        ...
        # Reset all team buttons to default style
